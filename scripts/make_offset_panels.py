...
from evaluation.offset_correction import (ridge_fit_predict,    # noqa: E402
                                          subregion_means)
...
def main(archs, day, zarr):
    ...
    for arch in archs:
        ...
        # Residual on EVERY day: the k=6 estimator is fit on the train split
        # and applied to test, so the fit must see the train days too.
        n_all = ds.sizes["time"]
        resid = np.empty((n_all, ds.sizes["y"], ds.sizes["x"]), "float32")
        for k in range(n_all):
            resid[k] = np.nan_to_num(ds["target"].isel(time=int(k)).values[0]) - predict(k)
        off_tr_all = resid[:, m_train].mean(1)
        off_ho_all = resid[:, m_hold].mean(1)
        ny_, nx_ = land.shape
        X6 = subregion_means(resid, m_train, ny_, nx_, k=6)
        pred6_all = ridge_fit_predict(X6, off_ho_all, np.where(split == "train")[0])

        off_tr, off_ho = off_tr_all[te], off_ho_all[te]
        pred6 = pred6_all[te]
        rr = float(np.corrcoef(off_tr, off_ho)[0, 1])
        r6 = float(np.corrcoef(pred6, off_ho)[0, 1])
        ...
        for n_, k_ in enumerate(te):
            ...
            yr_c += np.mean((p_ + pred6[n_] - t_)[m_hold] ** 2)
            yr_g += np.mean((p_ + off_tr[n_] - t_)[m_hold] ** 2)
            yr_o += np.mean((p_ + off_ho[n_] - t_)[m_hold] ** 2)
            yr_i += np.mean((c_ + clim_c[doy[k_]] - (t_ + clim[doy[k_]]))[m_hold] ** 2)
        Y = [np.sqrt(v / len(te)) for v in (yr_i, yr_m, yr_g, yr_c, yr_o)]
        coarse = ds["input"].isel(time=kday).values[stored.index("coarse_tmp")] + cc
        truth, pred = truth_a + ct, pred_a + ct
        corr_field = pred + pred6[kk]          # k=6 sub-regional estimate
        glob_field = pred + off_tr[kk]         # the single-mean estimator
        ...
        for a, f, t, sub in [
            ...
            (ax[0, 3], corr_field, "Offset-corrected (k=6 sub-regions)",
        ...
        sc = ax[1, 2]
        lim = float(np.nanpercentile(np.abs(np.r_[off_tr, off_ho, pred6]), 99.5))
        sc.scatter(off_tr, off_ho, s=9, alpha=.30, color="#9ec5f4",
                   edgecolors="none", label=f"global mean, r={rr:.2f}")
        sc.scatter(pred6, off_ho, s=9, alpha=.60, color="#2a78d6",
                   edgecolors="none", label=f"k=6 estimate, r={r6:.2f}")
        sc.plot([-lim, lim], [-lim, lim], ls="--", lw=1, color=INK3, label="perfect")
        sc.scatter([pred6[kk]], [off_ho[kk]], s=70, facecolor="none",
        ...
        sc.set_xlabel("ESTIMATED offset (training region only), degC",
                      fontsize=8.5, color=INK2)
        sc.set_ylabel("true offset in the HELD-OUT region, degC",
                      fontsize=8.5, color=INK2)
        sc.set_title(f"Why it works: r {rr:.2f} -> {r6:.2f}", fontsize=11.5,
                     fontweight="semibold", pad=10)
        sc.text(.5, 1.004, f"{len(te)} test days; neither estimator uses holdout truth",
        ...
        items = [("interpolation", Y[0], "#c9c6bf"), ("model", Y[1], "#9aa5b1"),
                 ("+ single global mean", Y[2], "#9ec5f4"),
                 ("+ k=6 sub-regions", Y[3], "#2a78d6"),
                 ("oracle offset", Y[4], "#eb6834")]
        ...
        lad.set_title(f"Whole test year: {(Y[3] - Y[1]) / Y[1] * 100:+.1f}%"
                      f"   ({(Y[3] - Y[1]) / (Y[4] - Y[1]) * 100:.0f}% of oracle)",
        ...
        fig.text(.012, .958,
                 ...
                 f"number per day, PREDICTED from 36 block means of the model's "
                 f"residual over the training region (ridge, fit on the train "
                 f"split) — it never sees held-out truth. This is the MEDIAN-offset "
        ...
        plt.close(fig)
        print(f"  {out.name}   YEAR: model {Y[1]:.3f} | global {Y[2]:.3f} | "
              f"k=6 {Y[3]:.3f} ({(Y[3] - Y[1]) / Y[1] * 100:+.1f}%, "
              f"{(Y[3] - Y[1]) / (Y[4] - Y[1]) * 100:.0f}% of oracle) | "
              f"oracle {Y[4]:.3f} | r {rr:.2f}->{r6:.2f}")
